...
public class PessoaService {
    ...
    // READ by CPF/CNPJ
    @Transactional(readOnly = true)
    public PessoaResponse getByCpfCnpj(String Cpf) {
        Pessoa p = repository.findByCpf(Cpf)
                .orElseThrow(() -> new IllegalArgumentException("Pessoa não encontrada. cpfCnpj=" + Cpf));
        return toResponse(p);
    }
    ...
    // ---------- Helpers ----------
    private void validarUnicidadeCpfCnpj(String Cpf, Long idAtual) {
        repository.findByCpf(Cpf).ifPresent(existente -> {
            if (idAtual == null || !existente.getId().equals(idAtual)) {
                throw new DataIntegrityViolationException("CPF/CNPJ já cadastrado: " + Cpf);
            }
        });
    }
